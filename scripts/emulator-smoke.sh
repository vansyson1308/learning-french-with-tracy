...
for m in re.finditer(r'<node [^>]*>', xml):
    ...
    hay = ((text.group(1) if text else "") + "\n" + (desc.group(1) if desc else "")).lower()
    if want in hay:
        b = re.search(r'bounds="\[(\d+),(\d+)\]\[(\d+),(\d+)\]"', node)
        if b:
            x1,y1,x2,y2 = map(int, b.groups()); area=(x2-x1)*(y2-y1)
            if best is None or area < best[0]: best = (area, (x1+x2)//2, (y1+y2)//2)
if not best: print("NOTFOUND"); sys.exit(1)
subprocess.run(["adb","shell","input","tap",str(best[1]),str(best[2])], check=False); print(f"tapped {want} at {best[1]},{best[2]}")
PY
}
...
tap_text "French" onb1 >/dev/null; sleep 1
tap_text "Learn French" onb2 >/dev/null; sleep 2
if wait_text "Studied French before?" 40 learn; then row "onboarding → Learn tab (French)" PASS "'Studied French before?' visible"; else row "onboarding → Learn tab (French)" FAIL "Learn tab not reached"; fi
shot 02-learn-tab
check_screen() { # route, expected text, name
  deeplink "$1"; sleep 2
  if wait_text "$2" 40 "$3"; then row "deep link $1" PASS "'$2' visible"; else row "deep link $1" FAIL "'$2' not found (alive=$(alive && echo yes || echo no))"; fi
  shot "$3"
}
...
check_screen "licenses" "Piper" 07-licenses
check_screen "privacy" "Privacy" 08-privacy
check_screen "goals" "Goals" 09-goals
