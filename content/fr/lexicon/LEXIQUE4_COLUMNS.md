...
| 9 | `9_InfoVER` | string | 54.7% | Verb morphology as comma-joined `mood:tense:person` analyses (`ind:pre:1,imp:pre:1`), trailing empty person for non-personal forms (`par:pas:`). Observed moods: `inf`, `ind`, `imp` (imperative), `cnd`, `sub`, `par`; observed tenses: `pre`, `imp` (imparfait — the first field disambiguates: `imp:pre:*` is imperative present, `ind:imp:*` is indicative imparfait), `fut`, `pas`; the full inventory (46 atomic analyses) is recorded in `derived/verb-morphology.json`. **Two evidence-established caveats (from the real extraction):** (1) on high-frequency homographic forms the value is a **form-level union across lemmas** — the être-lemma row for "suis" also lists suivre's `imp:pre:2`/`ind:pre:2`, "sommes" lists sommer's readings; (2) `8_Nombre` is a row-level attribute that is **noisy on these rows** (observed: the "peux" row carries `p` though je/tu peux is singular), so number must never gate a per-analysis decision. **Consumption contract:** Phase 5B AUTHORS its conjugation cells and verifies each (form, cell) against this evidence (atom present on a row of the lemma with matching orthographic form) — never blind extraction; sufficiency for the full curriculum scope is pinned by `scripts/__tests__/verb-morphology-coverage.test.ts`. **Use:** §26 verb-morphology recovery — no additional conjugation dataset is needed. |
...
| 13 | `13_CDOrtho` | number | 0% | Contextual diversity of the orthographic form — the breadth of subtitle documents containing it (README names contextual diversity explicitly). **Scale confirmed against the real derivation: a percentage 0–100 of documents** (`derived/frequency-stats.json`: max 99.94; "pas" 99.80, "ne" 99.59, top-frequency lemmas all > 90). Measured per FORM, not per lemma (no `CDLemme` exists), so a verb lemma's own-form CD understates its real breadth — which is why CD is a stored breadth/quality signal and corroboration filter, while `12_FreqLemme` is the primary lemma-ranking variable. CD corroboration exposed two source lemmatization artifacts ("upas" carrying "pas"'s 18,098/M at CD 0.014; "garçonne" at 431.8/M, CD 0.037) — the candidate pool excludes such rows with a recorded reason (`failsCdCorroboration`). **Use:** candidate-pool quality guard + signal; §17 raw storage. |
...
| 33 | `33_Preval` | number | 39.1% | Word prevalence — the share of speakers who know the word, from prevalence norming (README lists prevalence). **Range confirmed 0–100** (`derived/frequency-stats.json`; present for 34,716 of the 64,836-entry eligible population). **Use:** candidate-pool quality signal (a frequent-but-unknown-to-raters form is a poor teaching candidate — the observed "upas" artifact carries prevalence 0, corroborating the CD guard); stored raw where present. |
