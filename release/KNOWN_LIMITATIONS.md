...
|---|---|---|
| Store identity — RESOLVED on 2026-09-08: the owner approved the personal identity (`com.vansyson1308.learningfrenchwithtracy`, slug `learning-french-with-tracy`) and it is applied; the inherited `com.ahmet.lingo` / EAS `c5e5ee9a…` / ASC `6781818623` / owner `ahmet909` values are gone from app.json and eas.json | remaining: the owner's own EAS project (`eas init`) and, later, the store records; store distribution stays fail-closed until `apply-identity.ts link --allow-store` | `identity.json`, `ANDROID_RC2_BASELINE.md`, `PUBLISHING_RUNBOOK.md` B/D |
...
| Hosted privacy-policy URL and support contact | the site deploys from `main` via GitHub Pages (enabled by the owner on 2026-09-08; deployment 6316645993 success on 2026-09-07); the support email `duymank250997@gmail.com` is set (provisional) and rendered on the support and privacy pages; the privacy URL counts as live only after the runner-side `verify` job observes HTTP 200 over HTTPS | `PUBLICATION_STATUS.md`, `PRIVACY_FINAL.md`, `release/support-contact.json` |
| Legacy audio provenance | **CLOSED** — every upstream recording was removed; German, Spanish, French and Portuguese audio and the sound effects are regenerated by the pinned, license-gated Piper pipeline (run 33648420215); Italian, Japanese, Korean and Chinese use device speech | `AUDIO_PROVENANCE_FINAL.md` |

## P2 — minor defects and measurable gaps

| # | Item | Detail | Plan |
|---|---|---|---|
| L1 | Four Section-1 prompts had no bundled audio | *C'est un garçon.* / *C'est une fille.* replaced ungrammatical sentences whose recordings existed | **CLOSED** — restored and voiced by the pack-audio pipeline (`restoreAudioTargets`, run 33648420215) |
