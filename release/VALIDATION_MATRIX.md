...
| Only the declared Android permissions ship; targetSdk 36 | APK badging/permissions dump from the built artifact | pass (RC1: run 33660049388, artifact 9859191410 — `RC_HISTORY.md`; re-run per RC) | `android-release-build.yml`, `RC_HISTORY.md` |
| The migrated app boots and navigates on Android (runtime, not just compile) | emulator smoke on a GitHub runner: install, cold launch, onboarding into French, seven deep-linked screens, background/foreground, crash/ANR/JS scan | pass (run 34169308900 on the new-identity build from run 34164812120) | `android-emulator-smoke.yml`, `scripts/emulator-smoke.sh`, `ANDROID_RC2.md` §2b |
