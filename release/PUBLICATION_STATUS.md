...
| Public website (privacy, support, guide, licenses, accessibility, release) | **WAITING FOR OWNER (one-time)**: the first deploy on `main` (Pages run 33663402880) built the 7 pages but the workflow token may not create the Pages site ("Create Pages site failed: Resource not accessible by integration"). Owner: Settings → Pages → Build and deployment → Source: **GitHub Actions**, then Actions → Pages → Run workflow (or push to `main`) | `pages.yml`, `scripts/build-site.ts` |
| Public privacy-policy URL | BLOCKED until the owner enables Pages and the re-run deploys (then verify `curl -sI https://vansyson1308.github.io/learning-french-with-tracy/privacy/` → 200) | `PRIVACY_FINAL.md` |
...
| RC builds (store profiles) | RC1 cut as an engineering candidate with its QA APK recorded (run 33660049388, artifact 9859191410, `RC_HISTORY.md`); the `v1.0.0-rc.1` tag is prepared but its push is refused to the automation credential (HTTP 403 on `refs/tags`) — owner pushes it (`RC_HISTORY.md`); store-profile builds BLOCKED by the identity gate (fails closed) and the EAS project | `RC_HISTORY.md` |
...
PUBLIC PRIVACY POLICY: BLOCKED (owner must enable GitHub Pages once — source: GitHub Actions — then the deploy publishes https://vansyson1308.github.io/learning-french-with-tracy/privacy/)
...
**ENGINEERING COMPLETE** (on `main` at `eaeb604`);
